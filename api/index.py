...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;font-family:Arial,sans-serif}
    ...
    .title{text-align:center;font-size:2.6rem;font-weight:800;margin-bottom:10px}
    .subtitle{text-align:center;opacity:.9;margin-bottom:24px}
    .card{
    ...
    .kpi .v{font-size:2rem;font-weight:800;margin-top:6px;color:#8ed8ff}
    .divider{height:1px;background:rgba(255,255,255,.15);margin:16px 0}
    .notice{margin-top:12px;font-size:.92rem;opacity:.88}
  ...
  <div class="container">
    <div class="topbar">
      <div style="width:100%">
        ...
        <div class="subtitle">Versão sem FastAPI</div>
      </div>
  ...
  <script>
    ...
    async function initSession(){
      ...
      if(savedSession){
        ...
        sessionData = await api("/api/session/new", "POST");
    ...
    async function enviarQuestionario(){
      ...
      const res = await api("/api/answers", "POST", {
    ...
    async function loadObras(){
      ...
      const tagsGlobais = await api("/api/tags");
    ...
    async function loadAdmin(){
      const [tags, users, obras] = await Promise.all([
        api("/api/tags"),
        api("/api/users"),
        api("/api/obras")
      ]);
    ...
    async function carregarConexoes(){
      ...
      const conns = await api("/api/connections", "POST", {
        tags: lista,
        threshold: 0.35
      });
...
def html_response(html, status="200 OK"):
...
def app(environ, start_response):
    ...
    try:
        if path == "/" and method == "GET":
        ...
        elif path == "/api/session/new" and method == "POST":
            status, headers, body = json_response({
                "user_id": gen_uid(),
                "animal_name": generate_animal_name()
            })

        elif path == "/api/obras" and method == "GET":
            status, headers, body = json_response(load_obras())

        elif path == "/api/tags" and method == "GET":
            status, headers, body = json_response(all_tags())

        elif path == "/api/users" and method == "GET":
            status, headers, body = json_response(all_users())

        elif path == "/api/answers" and method == "POST":
            payload = read_json_body(environ)
            required = ["user_id", "animal_name", "q1", "q2", "q3"]
            if not all(k in payload for k in required):
                status, headers, body = json_response({"ok": False, "error": "Campos ausentes"}, "400 Bad Request")
            else:
        ...
        elif path == "/api/tags" and method == "POST":
            payload = read_json_body(environ)
            required = ["user_id", "obra_id", "tag"]
            if not all(k in payload for k in required):
                status, headers, body = json_response({"ok": False, "error": "Campos ausentes"}, "400 Bad Request")
            else:
                save_tag(payload["user_id"], payload["obra_id"], payload["tag"])
                status, headers, body = json_response({"ok": True})
            ...
            if check_login(username, password):
                ...
                status, headers, body = json_response({"ok": False, "error": "Credenciais inválidas"}, "401 Unauthorized")

        elif path == "/api/connections" and method == "POST":
            payload = read_json_body(environ)
            tags = payload.get("tags", [])
            threshold = float(payload.get("threshold", 0.35))
            status, headers, body = json_response(tag_connections(tags, threshold))

        elif path == "/ping" and method == "GET":
            status, headers, body = json_response({"ok": True})

        else:
            status, headers, body = json_response({"ok": False, "error": "Not found"}, "404 Not Found")

    except Exception as e:
        status, headers, body = json_response({"ok": False, "error": str(e)}, "500 Internal Server Error")
